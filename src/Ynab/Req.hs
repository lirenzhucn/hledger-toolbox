...
instance (FromJSON p) => FromJSON (PayloadWrapper p) where
    parseJSON = genericParseJSON defaultOptions
        { omitNothingFields = True, fieldLabelModifier = fieldNameMap }
        where
...
-- build get request
buildWrappedGETRequest :: (MonadHttp m, FromJSON d) =>
    Url scheme -> Option scheme -> m (JsonResponse (PayloadWrapper d))
